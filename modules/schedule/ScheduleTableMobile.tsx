    ...
    return <div className="flex flex-col gap-[10px]">
        {
            Object.entries(DAY_NAMES).map(([weekday, weekdayName], index) => {
                return <div key={weekday}
                            className={clsx("rounded-md bg-background shadow-sm flex flex-col gap-[10px] p-[10px] pb-[5px]", isToday(addDaysToDate(weekStartDate, index)) && "outline-primary outline")}>
                    ...
                    <div className="flex flex-col gap-[6px]">
                        {
                            schedule[weekday].length > 1 ?
                                schedule[weekday].map((pair) => (
                                    <ScheduleTableMobileItem key={`${weekday}-${pair.id}`} title={pair.name}
